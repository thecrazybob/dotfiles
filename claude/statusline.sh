...
session_cost_raw=$(echo "$input" | jq -r '.cost.total_cost_usd // "null"')
if [ "$session_cost_raw" != "null" ] && [ -n "$session_cost_raw" ] && [ "$session_cost_raw" != "0" ]; then
    session_cost=$(printf "%.4f" "$session_cost_raw")
else
    session_cost=""
fi
...
# Get git branch (--no-optional-locks prevents index.lock conflicts with concurrent git operations)
if git --no-optional-locks rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    # Get git status with file counts (--no-optional-locks prevents index.lock conflicts)
